...
INPUT:
    ...
    terms:= the terms {eps_, p_j, pi_j, (pi_)_j} appearing Kappa*, excluding p0, where
        Kappa*:= nstar*log_p(eps_) - sum_{j in IUstar} ustar_j*log_p( p_j ) + sum_{j in I} cstar_j*log_p( (pi_)_j ) + sum_{j in I_} cstar_j*log_p( pi_j )
    p0:= the term of Kappa* giving minimal ord_p(log_p(p0))
    w:= the set of all lattice coefficient vectors x_i of the lattice L such that y_i:= Transpose(W*Transpose(x_i)) has norm <= C,
        ...
        y_i:= row vector whose entries give nstar, cstar_j, ustar_j of Kappa*, where
            Kappa*:= nstar*log_p(eps_) - sum_{j in IUstar} ustar_j*log_p( p_j ) + sum_{j in I} cstar_j*log_p( (pi_)_j ) + sum_{j in I_} cstar_j*log_p( pi_j )
...
COMMENTS:
    Computes all S-unit equations x + y = z^2 involving c_j in the range [(m + m0 - kappa_i - Ord_p(hstar))/h_i, M0[p]], for given prime p in M0:
        For a given prime p, to find all solutions (x,y,z) coprime with c_j <= M0[p]:
            Choose m < (h_i)*M0[p] - m0 + kappa_i + Ord_p(hstar) and consider the lattice Kappa_{m}* = KappaLatticeXYZ2(A, IUstar,p, P, m, D);
            If a solution (x,y,z) exists with c_j in [(m + m0 - kappa_i - Ord_p(hstar))/h_i, M0[p]], then the vector (x_1*,...,x_s*,x_0*)^T is in the lattice, where x_i*:= n*, c_j*, or u_j* and
                n:= (n* - n_0)/hstar, c_j:= (c_j* - n_j)/hstar, u_j:= (u_j* + (n_j - v_j))/hstar
                 such that
                    n:= an integer representing the exponent on eps
                    c_j:= the exponents m_j on Prod[I], Prod_[I] in G_{alpha} giving G_{alpha} = u, under x = D*u^2,
    ...
    This algorithm uses the Fincke-Pohst algorithm to find all vectors of length at most ((absn*)^2 + [(c_j*)^2 | j in I] + [(c_j*)^2 | j in I_] + [(u_j*)^2 | j in IU*]) and
        sorts them to give (x,y,z) for the specified p in M0
...
EXAMPLE:
    ...
    > maxReached;
    false
    > soln:= FPRestrictionsKappaXYZ2(A,U0,M0,M0_,absn,n,nM,nM_,nEps,hstar,terms,p0,w,W,S,D);
    > soln;
    [
        [ 199290375, -686, 14117 ],
        [ 375, -14, 19 ],
...
function FPRestrictionsKappaXYZ2(A,U0,M0,M0_,absn,n,nM,nM_,nEps,hstar,terms,p0,w,W,S,D);
    ...
    Append(~terms, p0);         // computes the terms {eps_, p_j, pi_j, (pi_)_j} appearing Kappa*, including p0, where
                                    // Kappa*:= nstar*log_p(eps_) - sum_{j in IUstar} ustar_j*log_p( p_j ) + sum_{j in I} cstar_j*log_p( (pi_)_j ) + sum_{j in I_} cstar_j*log_p( pi_j )
    ...
    epsIndex:= Index(terms, eps_);      // computes the index of eps_ in terms
    IIndices:= [];      // stores [ < pi_j, (pi_)_j, Index of (pi_)_j, M0[j] , nM[j] >: j in I ]; Index of (pi_)_j is the index of (pi_)_j as it appear in terms;
                            // M0[j] is the upper bound on the exponent of this term, as in M0; nM[j] is the exponent n_j corresponding to c_j
    I_Indices:= [];     // stores [ < pi_j, (pi_)_j, Index of pi_j, M0_[j] , nM_[j] >: j in I_ ]; Index of pi_j is the index of pi_j as it appear in terms;
    ...
    for j in I do
        Append(~IIndices, < j[3], j[4], Index(terms, j[4]), M0[ Index(Ms,j[1]) ][1], nM[Index(Ms,j[1]) ][2] > );
    end for;
    for j_ in I_ do
        Append(~I_Indices, < j_[3], j_[4], Index(terms, j_[3]), M0_[ Index(M_s,j_[1]) ][1], nM_[Index(M_s,j_[1]) ][2] > );
    end for;
    /* for i in U do
    ...
    VectorSort1:= [];   // stores only the vectors m_i = w[i] found in Fincke-Pohst such that each vector element m_i[j] correspond to elements of the lattice Kappa
                            // ie. the vector element m_i[j] of m_i corresponding to n*, c_j*, u_j* of Kappa* should necessarily also correspond to n, c_j, u_j of Kappa;
    for m in w do       // checks each vector w[j] found in Fincke-Pohst, weeding them out accordingly
        m_i:= Transpose(W*Transpose(m));        // computes the short vector m_i of the lattice Kappa*
