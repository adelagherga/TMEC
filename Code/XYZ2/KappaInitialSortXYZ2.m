...
function KappaInitialSortXYZ2(A, IUstar, p, P, mu, D)
    ...
    terms:= [ eps_ ];    // stores all terms appearing in the linear form in logarithm, Kappa*
    P0:= [Valuation(pAdicLog( (Qp!eps_[1] + Qp!eps_[2]*SqrtDModp) ,p) )];        // computes ord_p(log_p(p_i)) for eps_ as a p-adic integer
    
    for q in I do
        Append(~terms, q[4]);
        Append(~P0, Valuation(pAdicLog( (Qp!q[4][1] + Qp!q[4][2]*SqrtDModp) ,p) ) );    // computes ord_p(log_p(p_i)) for each term (pi_)_j, j in I, as a p-adic integer
    end for;
    for q_ in I_ do
        Append(~terms, q_[3]);
        Append(~P0, Valuation(pAdicLog( (Qp!q_[3][1] + Qp!q_[3][2]*SqrtDModp) ,p) ) );  // computes ord_p(log_p(p_i)) for each term pi_j, j in I_, as a p-adic integer
    end for;
